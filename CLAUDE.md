...
**4 Sep (later) — VICTUS WITHDRAWN; A4 prepared as one Kaggle evaluation session, awaiting Rohit's push.** Rohit: the Victus cannot be arranged, so Track B continues on Kaggle and this laptop only (plan §7a amendment 3, which supersedes the Victus carve-out for A4–A6 placement). Placement follows measured cost: this laptop's CPU runs the five backbones at 2–16 images/s, so the 80-cell cross-generator sweep would take ~15 h and the five Grad-CAM prediction passes ~7 h, against well under 2 h on a T4 — both go to Kaggle; the laptop keeps development, merging, A6 consolidation and the checkpoint archive. **No checkpoint upload is needed:** the eight A3 session notebooks' outputs already hold every run dir, and the A4 notebook attaches them as kernel data sources beside CIFAKE and `yangsangtai/tiny-genimage`. Built and CPU-verified this pass: `results/canonical_runs.txt` (the A6 manifest, created early because A4 needs the same list; the three 50-epoch reruns selected), `code/run_crossgen.py` (records tiny-genimage's per-generator counts, file formats and image sizes to `genimage_verification.csv`, closing the A1 `[VERIFY]`, then runs 80 failure-isolated, resumable `eval.py` cells), `code/make_galleries.py` (better checkpoint per backbone by val_loss; shared correct set via dump → intersect → seeded sample → `gradcam_shared_list.txt`; then the five galleries), `eval.py` helpers `read_manifest`/`find_checkpoints`, and `notebooks/make_a4_notebook.py` → `phase_a4_crossgen_gradcam.ipynb` (7 cells; asserts all ten canonical checkpoints are mounted before spending anything). Local tests: a 16-cell sweep on a synthetic GenImage tree with two real checkpoints, a resume run that skips all 16, a crossgen merge that refuses duplicates, and galleries on a 50-image test subset whose correct panels are identical across models. **Not launched — the push is Rohit's** (`make_a4_notebook.py --push --token`, after toggling `GITHUB_PAT` on the new notebook). Two things only the launch can verify: that Kaggle accepts eight kernel sources plus two datasets on one notebook, and where kernel outputs mount — cell 2 autodetects and stops in seconds if either fails. A5's three ablation training runs stay on Kaggle T4 later; A6 runs on the laptop.
